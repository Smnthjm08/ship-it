...
This project is **not** on stock shadcn defaults. It runs an Expo-derived system documented in [DESIGN.md](DESIGN.md) and wired into `apps/web/app/globals.css` as CSS variables. The previous Cal.com-derived spec is kept at [OLD_DESIGN.md](OLD_DESIGN.md) for reference only — it is no longer the source of truth.
...
- **Type** — Inter for display *and* body, split by weight (600 / 400) and negative tracking rather than by family; JetBrains Mono for code and every machine-generated value. `.text-display-mega` → `.text-nav-link` scale plus `.text-eyebrow` and `.font-machine` in `globals.css`
